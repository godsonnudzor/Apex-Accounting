...
const permissionLabels = [
  {
    key: "dashboard",
    label: "Dashboard",
    description: "View financial overview and activity",
  },
  {
    key: "writeCheque",
    label: "Write cheque",
    description: "Create cash and cheque transactions",
  },
  {
    key: "payroll",
    label: "Payroll",
...
function Setting() {
  ...
  useEffect(() => {
    fetch(getApiUrl("/api/users"), { credentials: "include" })
      ...
      .then((result) => {
        setEmployees(result);
        if (result[0]) setSelectedId(String(result[0].id));
      })
  ...
  const selectedPermissions = selectedEmployee
    ? savedPermissions[selectedEmployee.id] || getPermissions(selectedEmployee)
    : defaults;
    ...
    <main className="settings-app">
      <header className="settings-topbar">
        <div>
          ...
          <p className="settings-eyebrow">Apex / administration</p>
          <h1>Access settings</h1>
          <p>Choose which screens each employee can open and work on.</p>
        </div>
        <span className="settings-admin-badge">Admin workspace</span>
      </header>
      {error ? (
      ...
      <div className="settings-layout">
        <aside className="settings-people">
          ...
          {employees.map((employee) => (
            <button
              className={`settings-person ${selectedId === String(employee.id) ? "selected" : ""}`}
              key={employee.id}
            ...
            >
              ...
              <span>
                ...
                <small>
                  {employee.role === "admin" ? "Administrator" : employee.email}
                </small>
              </span>
              <b>›</b>
            </button>
        ...
        {selectedEmployee ? (
          <section className="settings-content">
            <div className="settings-profile">
              <span className="settings-avatar large">
            ...
            <div className="settings-section-title">
              <div>
                ...
                <p>Changes save immediately to the employee account.</p>
              </div>
              <span>
                {Object.values(selectedPermissions).filter(Boolean).length} of{" "}
                {permissionLabels.length} enabled
              </span>
            </div>
            <div className="permission-list">
              {permissionLabels.map((permission) => (
                <div className="permission-row" key={permission.key}>
                  ...
                  <button
                    className={`permission-toggle ${selectedPermissions[permission.key] ? "on" : ""}`}
                    aria-pressed={selectedPermissions[permission.key]}
            ...
            <div className="settings-note">
              ...
              <p>
                Administrators always retain access to every workspace.
                Employees only receive the permissions enabled here.
              </p>
            </div>
          </section>
        ) : (
