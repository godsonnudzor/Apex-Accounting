      ...
      <main className="min-h-screen bg-slate-100 p-4 md:p-8">
        <div className="mx-auto max-w-7xl">
          <div className="mb-6">
            ...
            <p className="mt-2 text-sm text-slate-500">Condensed monthly view from posted journal entries.</p>
          </div>
          ...
          <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            <div className="flex min-h-56 flex-col items-center justify-center rounded-lg border border-slate-200 bg-slate-50 p-8 text-center"><p className="text-xs font-semibold uppercase tracking-widest text-slate-500">{tab === "profit-loss" ? "Net profit / loss" : "Balance sheet total"}</p><p className={`mt-3 text-5xl font-bold ${Number(total) >= 0 ? "text-emerald-700" : "text-red-700"}`}>{money(total)}</p><button type="button" className="mt-5 rounded bg-teal-600 px-4 py-2 text-sm font-semibold text-white hover:bg-teal-700" onClick={() => showReportDetails(tab)}>View entry key</button><p className="mt-2 text-xs text-slate-500">{tab === "profit-loss" ? "Income and expense entries" : "Asset, liability, and equity entries"}</p></div>
          </section>
